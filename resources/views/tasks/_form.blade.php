...
<section>
    ...
    <p class="mt-1 text-sm text-slate-500">
        Organize pendências, próximos passos e responsabilidades.
    </p>
    ...
    <div class="mt-4 grid gap-5 sm:grid-cols-2">
        <div class="sm:col-span-2">
            ...
            <input
                ...
                placeholder="Ex.: Retornar contato com a clínica"
            >
        </div>
        ...
        <div>
            <label class="label" for="due_at">Prazo</label>
        ...
        <div>
            <label class="flex items-center gap-3">
                <input
                    ...
                    name="is_follow_up"
                    value="1"
                    @checked(old('is_follow_up', $task?->is_follow_up))
                >

                <span class="font-medium text-slate-900">
                    Esta tarefa é um follow-up
                </span>
            </label>

            <p class="mt-2 text-sm text-slate-500">
                Follow-ups exigem prazo, canal e vínculo comercial.
            </p>
        </div>

        <div>
            <label class="label" for="follow_up_channel">
                Canal do follow-up
            </label>
...
<section class="mt-8 border-t border-slate-200 pt-7">
    ...
    <p class="mt-1 text-sm text-slate-500">
        Opcional. A tarefa também pode ser uma pendência interna.
    </p>
    ...
    <div class="mt-4 grid gap-5 sm:grid-cols-2">
            ...
            <select class="input" id="company_id" name="company_id">
                ...
                @foreach($companies as $company)
                    <option
                        value="{{ $company->id }}"
                        @selected(
                            (string) old('company_id', $task?->company_id)
                            === (string) $company->id
                        )
                    >
            ...
            <select class="input" id="contact_id" name="contact_id">
                ...
                @foreach($contacts as $contact)
                    <option
                        value="{{ $contact->id }}"
                        @selected(
                            (string) old('contact_id', $task?->contact_id)
                            === (string) $contact->id
                        )
                    >
                        {{ $contact->name }}
            ...
            <select class="input" id="lead_id" name="lead_id">
                ...
                @foreach($leads as $lead)
                    <option
                        value="{{ $lead->id }}"
                        @selected(
                            (string) old('lead_id', $task?->lead_id)
                            === (string) $lead->id
                        )
                    >
        ...
        <div>
            ...
            <select class="input" id="opportunity_id" name="opportunity_id">
                <option value="">Não vinculada</option>
...
<section class="mt-8 border-t border-slate-200 pt-7">
    ...
    <div class="mt-4">
        <textarea
            ...
            name="description"
            maxlength="10000"
            placeholder="Detalhes da tarefa, contexto e próximos passos..."
