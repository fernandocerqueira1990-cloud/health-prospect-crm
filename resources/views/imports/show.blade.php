<x-layouts.app title="Resultado da importação">
    ...
    <div class="card max-w-3xl">
        <dl class="grid gap-5 sm:grid-cols-2">
            ...
            <div><dt class="text-sm font-medium text-slate-500">Status</dt><dd class="mt-1">{{ ['uploaded' => 'Enviado', 'processing' => 'Processando', 'parsed' => 'Interpretado', 'completed' => 'Concluído', 'failed' => 'Falhou'][$dataImport->status] ?? $dataImport->status }}</dd></div>
        ...
        @endif
        @if(isset($dataImport->metadata['execution']))
            @can('view', $dataImport)<div class="mt-6 border-t border-slate-200 pt-6"><a class="btn-primary" href="{{ route('imports.report', $dataImport) }}">Ver relatório da importação</a></div>@endcan
        @endif
