...
<div class="space-y-4">
    <x-form-section title="Dados principais" description="Identificação e perfil da organização.">
        <div class="form-grid form-grid-4">
            <div class="md:col-span-2"><label class="label" for="legal_name">Razão social *</label><input class="input" id="legal_name" name="legal_name" maxlength="255" required value="{{ old('legal_name', $company?->legal_name) }}"></div>
            <div class="md:col-span-2"><label class="label" for="trade_name">Nome fantasia</label><input class="input" id="trade_name" name="trade_name" maxlength="255" value="{{ old('trade_name', $company?->trade_name) }}"></div>
            <div><label class="label" for="tax_id_country">País do documento fiscal</label><input class="input" id="tax_id_country" name="tax_id_country" maxlength="2" list="tax-id-countries" value="{{ old('tax_id_country', $company ? $company->tax_id_country : 'BR') }}" placeholder="BR"><datalist id="tax-id-countries"><option value="BR">Brasil</option><option value="CL">Chile</option><option value="AR">Argentina</option><option value="US">Estados Unidos</option><option value="MX">México</option></datalist><p class="helper-text">Código ISO de duas letras. Obrigatório quando houver documento.</p></div>
    ...
    <div class="grid gap-4 xl:grid-cols-2">
        <x-form-section title="Contato" description="Canais institucionais da empresa.">
            <div class="form-grid">
                ...
                <div class="sm:col-span-2"><label class="label" for="website">Website</label><input class="input" id="website" name="website" maxlength="2048" value="{{ old('website', $company?->website) }}" placeholder="https://exemplo.com"></div>
            </div>
        </x-form-section>

        <x-form-section title="Informações comerciais" description="Responsabilidade, prioridade e porte.">
    ...
    <x-form-section title="Localização" description="Endereço comercial da organização.">
        <div class="form-grid form-grid-4">
            <div class="md:col-span-2"><label class="label" for="street">Logradouro</label><input class="input" id="street" name="street" maxlength="255" value="{{ old('street', $company?->street) }}"></div>
    ...
    <x-form-section title="Observações" description="Contexto adicional relevante para o relacionamento.">
        <label class="sr-only" for="notes">Observações</label><textarea class="input min-h-24" id="notes" name="notes" maxlength="10000">{{ old('notes', $company?->notes) }}</textarea>
    </x-form-section>

    <div class="form-actions"><a class="btn-secondary" href="{{ $company ? route('companies.show', $company) : route('companies.index') }}">Cancelar</a><button class="btn-primary" type="submit">Salvar empresa</button></div>
</div>
